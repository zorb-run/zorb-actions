...
/**
 * Load env vars from one or more `.env` files. Each `KEY=VALUE` pair is
 * registered into the run-scoped env table via `context.setEnv`, making it
 * visible to subsequent steps as both a subprocess env var and via
 * `${{ env.<name> }}` in later expressions.
 *
...
export async function action(rawInputs: ActionInputs, context: ActionContext): Promise<void> {
  ...
  for (const rel of paths) {
    ...
    for (const [name, value] of parseDotenv(text)) {
      ...
      context.setEnv(`${prefix}${name}`, value);
    }
  }
}
...
async function readFileOrMissing(
  ...
  try {
    ...
    if (isENoEnt(err)) {
      if (required) {
        throw new Error(`@zorb/env/load-dotenv: file not found: ${rel} (resolved to ${abs})`);
      }
      context.log.warn(`@zorb/env/load-dotenv: skipped missing file: ${rel}`);
      return undefined;
    }
    throw err;
  }
