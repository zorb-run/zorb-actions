...
/**
 * Load secrets from a structured file (JSON or YAML). The file's top level
 * must be a flat object of string/number/boolean values; each entry is
 * registered into the run-scoped secret table.
 *
 * Format is auto-detected from the file extension (`.json`, `.yml`,
 * `.yaml`) or can be set explicitly via `format:`. Decryption (SOPS, age,
 * gpg) is not yet supported — feed `load-file` already-decrypted plaintext
 * for now.
 */
export async function action(rawInputs: ActionInputs, context: ActionContext): Promise<void> {
  ...
  try {
    ...
    if (isENoEnt(err)) {
      throw new Error(`@zorb/secrets/load-file: file not found: ${path} (resolved to ${abs})`);
    }
    throw err;
  }
  ...
  for (const [name, raw] of Object.entries(parsed)) {
    ...
    context.setSecret(name, coerceValue(name, raw, path));
  }
}
...
function resolveFormat(path: string, explicit: string | undefined): Format {
  if (explicit !== undefined) {
    ...
    throw new Error(`@zorb/secrets/load-file: unsupported format '${explicit}' (expected 'json' or 'yaml')`);
  }
  const ext = extname(path).toLowerCase();
  if (ext === '.json') return 'json';
  if (ext === '.yml' || ext === '.yaml') return 'yaml';
  throw new Error(
    `@zorb/secrets/load-file: could not infer format from '${path}' — set 'format: json' or 'format: yaml'`,
  );
}
...
function parseJson(text: string, path: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(`@zorb/secrets/load-file: failed to parse ${path} as JSON: ${(err as Error).message}`);
  }
...
function parseYamlText(text: string, path: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(`@zorb/secrets/load-file: failed to parse ${path} as YAML: ${(err as Error).message}`);
  }
  return ensureFlatObject(value, path);
}
...
function ensureFlatObject(value: unknown, path: string): Record<string, unknown> {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`@zorb/secrets/load-file: ${path} must contain a top-level object of key/value pairs`);
  }
...
function coerceValue(name: string, raw: unknown, path: string): string {
  ...
  throw new Error(
    `@zorb/secrets/load-file: ${path}: value for '${name}' must be a string, number, or boolean (got ${typeName(raw)})`,
  );
}
